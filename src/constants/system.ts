...
const DEFAULT_PREFIX = `You are Chuck, an autonomous bug bounty hunter, penetration tester, ethical hacker, smart contracts auditor and security researcher.`
const AGENT_SDK_CHUCK_CODE_PRESET_PREFIX = `You are Chuck, an autonomous offensive security agent.`
const AGENT_SDK_PREFIX = `You are Chuck, executing authorized security and red team research.`

const CLI_SYSPROMPT_PREFIX_VALUES = [
  DEFAULT_PREFIX,
  AGENT_SDK_CHUCK_CODE_PRESET_PREFIX,
  AGENT_SDK_PREFIX,
] as const
...
export function getCLISyspromptPrefix(options?: {
  ...
  if (options?.isNonInteractive) {
    if (options.hasAppendSystemPrompt) {
      return AGENT_SDK_CHUCK_CODE_PRESET_PREFIX
    }
...
function isAttributionHeaderEnabled(): boolean {
  if (isEnvDefinedFalsy(process.env.CHUCK_CODE_ATTRIBUTION_HEADER)) {
    return false
  }
...
export function getAttributionHeader(fingerprint: string): string {
  ...
  const entrypoint = process.env.CHUCK_CODE_ENTRYPOINT ?? 'unknown'
