...
import { checkAndRefreshOAuthTokenIfNeeded, getChuckAIOAuthTokens } from './auth.js';
...
You should keep it short and simple, ideally no more than 4 words. The branch should always start with "chuck/" and should be all lower case, with words separated by dashes.

Return a JSON object with "title" and "branch" fields.

Example 1: {"title": "Fix login button not working on mobile", "branch": "chuck/fix-mobile-login-button"}
Example 2: {"title": "Update README with installation instructions", "branch": "chuck/update-readme"}
Example 3: {"title": "Improve performance of data processing script", "branch": "chuck/improve-data-processing"}
...
/**
 * Generates a title and branch name for a coding session using Chuck Haiku
...
async function generateTitleAndBranch(description: string, signal: AbortSignal): Promise<TitleAndBranch> {
  ...
  const fallbackBranch = 'chuck/task';
  try {
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    const accessToken = getChuckAIOAuthTokens()?.accessToken;
    if (!accessToken) {
      ...
      throw new Error('Chuck Code web sessions require authentication. Please run /login to authenticate, or check your authentication status with /status.');
    }
      ...
      case 'not_in_repo':
        {
          ...
          throw new TeleportOperationError(`You must run chuck-ai --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run chuck-ai --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          ...
          throw new TeleportOperationError(`You must run chuck-ai --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run chuck-ai --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
...
/**
 * Creates a remote Chuck.ai session with error handling and UI feedback.
...
export async function pollRemoteSessionEvents(sessionId: string, afterId: string | null = null, opts?: {
  ...
  const headers = {
    ...
    'chuck-beta': 'ccr-byoc-2025-07-29',
    'x-chuck-org-uuid': orgUUID
  };
...
/**
 * Creates a remote Chuck.ai session using the Sessions API.
 *
 * Two source modes:
...
export async function teleportToRemote(options: {
  ...
  try {
    ...
    // Select environment based on settings, then chuck_cloud preference, then first available.
    // Prefer chuck_cloud environments over byoc: chuck_cloud environments (e.g. "Default")
    ...
    let cloudEnv = environments.find(env => env.kind === 'chuck_cloud');
    ...
    if (options.useDefaultEnvironment && !cloudEnv) {
      logForDebugging(`No chuck_cloud in env list (${environments.length} envs); retrying fetchEnvironments`);
      const retried = await fetchEnvironments();
      cloudEnv = retried?.find(env => env.kind === 'chuck_cloud');
      if (!cloudEnv) {
